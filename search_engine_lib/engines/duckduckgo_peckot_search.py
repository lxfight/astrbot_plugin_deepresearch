...
class DuckDuckGoPeckotSearch(BaseSearchEngine):
    ...
    async def search(self, search_query: SearchQuery) -> SearchResponse:
            ...
            try:
                async with session.post(api_url, json=payload, headers=headers) as response:
                    ...
                    if data.get("code") != 200:
                        logger.error(f"[{self.name}] API返回错误: {data.get('message', 'Unknown error')}")
                        if "advice" in data:
                ...
                logger.error(f"[{self.name}] API请求HTTP错误: 状态码={e.status}, 信息={e.message}")
        ...
        end_time = time.time()
        logger.info(f"[{self.name}] 搜索完成, 共找到 {len(results_list)} 条结果, 耗时 {round(end_time - start_time, 4)} 秒")
