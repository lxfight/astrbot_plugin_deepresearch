    ...
    def _initialize_resolvers(self):
        ...
        for resolver_class in resolver_classes:
            try:
                ...
                logger.warning(f"[URLResolver] 初始化解析器失败 {resolver_class.__name__}: {e}")
            ...
            if resolver.can_resolve(url):
                ...
                try:
                    ...
                    if resolved_url and resolved_url != url:
                        logger.info(f"[URLResolver] URL解析成功: {url} -> {resolved_url}")
                        return resolved_url
                except Exception as e:
                    logger.warning(f"[URLResolver] 解析器 {resolver.name} 处理失败: {e}")
                    continue
