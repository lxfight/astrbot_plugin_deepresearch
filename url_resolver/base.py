...
class BaseURLResolver(ABC):
    ...
    async def _follow_redirects(self, url: str, client: httpx.AsyncClient, max_redirects: int = 5) -> Optional[str]:
        """通用重定向跟踪方法"""
        try:
    ...
    async def _extract_from_response(self, url: str, client: httpx.AsyncClient) -> Optional[str]:
        """从响应内容中提取真实URL"""
        try:
            response = await client.get(url, follow_redirects=False, timeout=self.timeout)
