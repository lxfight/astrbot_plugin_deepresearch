...
@register_retriever("web", alias="bing", priority=9)
class BingSearchRetriever(BaseRetriever):
    """
    Bing Web Search API 检索器实现
    使用 Bing Search API 进行网页搜索
    """

    def __init__(self, context, config):
        super().__init__(context, config)
        self.bing_config = config.get("bing_search", {})
        self.api_key = self.bing_config.get("api_key", "")
        self.endpoint = self.bing_config.get(
            "endpoint", "https://api.bing.microsoft.com/v7.0/search"
        )
        self.default_params = {
            "count": min(self.max_results_per_engine, 10),
            "mkt": "zh-CN",
            "safeSearch": "Moderate",
        }
        self.rate_limit_per_minute = 100
        self.logger.info("BingSearchRetriever 模块初始化完成")

    def get_required_config_keys(self) -> Set[str]:
        return {"bing_api_key", "bing_endpoint"}

    def check_config_valid(self, api_config: Dict[str, Any]) -> bool:
        api_key = api_config.get("bing_api_key", self.api_key)
        endpoint = api_config.get("bing_endpoint", self.endpoint)
        if not api_key or not endpoint:
            self.logger.warning("Bing API Key 或 endpoint 未配置")
            return False
        if len(api_key) < 20:
            self.logger.warning("Bing API Key 格式可能不正确")
            return False
        if not endpoint.startswith("http"):
            self.logger.warning("Bing endpoint 格式可能不正确")
            return False
        return True
    ...
    async def search(
        ...
        if not self.check_config_valid(api_config):
            self.logger.error("Bing搜索配置无效，无法执行搜索")
            return []
        try:
            params = self.default_params.copy()
            params.update({"q": query})
            results = await self._execute_search_request(endpoint, api_key, params)
            ...
            self.logger.info(
                f"Bing搜索 '{query}' 完成，获得 {len(filtered_items)} 个质量结果"
            )
            return filtered_items
        except Exception as e:
            self.logger.error(f"Bing搜索出错: {e}", exc_info=True)
            return []
    ...
    async def _execute_search_request(
        ...
        async with aiohttp.ClientSession(
            ...
            async with session.get(
                endpoint, params=params, **request_kwargs
            ) as response:
                if response.status == 429:
                    raise Exception("Bing搜索API配额已用完或请求过于频繁")
                elif response.status == 401:
                    raise Exception("Bing搜索API访问被拒绝，请检查API Key")
                elif response.status != 200:
                    error_text = await response.text()
                    raise Exception(
                        f"Bing搜索API返回错误 {response.status}: {error_text}"
                    )
                try:
                    data = await response.json()
                    return data
                except json.JSONDecodeError as e:
                    raise Exception(f"Bing搜索API返回无效JSON: {e}")
    ...
    def _parse_search_results(
        ...
        try:
            ...
            self.logger.debug(f"Bing搜索解析了 {len(results)} 个结果")
        except Exception as e:
            self.logger.error(f"解析Bing搜索结果时出错: {e}", exc_info=True)
        return results
